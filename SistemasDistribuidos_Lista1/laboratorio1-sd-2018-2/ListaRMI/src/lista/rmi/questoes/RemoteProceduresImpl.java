...
public class RemoteProceduresImpl extends UnicastRemoteObject  implements RemoteProcedures, Serializable{
	...
	public JSONObject calculaPesoIdealJson(String altura, String sexo) throws RemoteException, JSONException {
		...
		if(sexo.equalsIgnoreCase("masculino")) {
			...
			resultado = "Seu peso ideal é: "+df.format(altura1)+" kg." ;
	...
	public JSONObject calculaCreditoJson(double saldoAnterior) throws RemoteException, JSONException {
		...
		if(saldoAnterior > 0 && saldoAnterior <= 200.0) {
			...
			resultado="Saldo atual do cliente: "+saldoAnterior+". Percentual de credito especial disponível: 20%. Novo saldo + credito:"+df.format(credito)+" ";
		} else if(saldoAnterior > 401 && saldoAnterior <= 601) {
			credito = saldoAnterior * 0.3 + saldoAnterior;
			resultado="Saldo atual do cliente: "+saldoAnterior+". Percentual de credito especial disponível: 30%. Novo saldo + credito:"+df.format(credito)+"";
		} else if (saldoAnterior > 601){
			credito = (saldoAnterior * 0.4) + saldoAnterior;
			resultado="Saldo atual do cliente: "+saldoAnterior+". Percentual de credito especial disponível: 40%. Novo saldo + credito:"+df.format(credito)+"";
		} else {
	...
	public JSONObject calculaAposentadoriaJson(int idade, int tempoServico, String sexo) throws RemoteException, JSONException {
		...
		if(sexo.equalsIgnoreCase("masculino")) {
			if(idade >= 65 && tempoServico >= 30) {
				resultado = " Trabalhador com "+idade+ " anos e tempo de Serviço "+tempoServico+ ". Resultado: apto para aposentadoria";
			} else {
				resultado = " Trabalhador com "+idade+ " anos e tempo de Serviço "+tempoServico+ ". Resultado: inapto para aposentadoria";
			}
		} else {
		
			if(sexo.equalsIgnoreCase("feminino")) {
				if(idade >= 60 && tempoServico >= 25) {
					resultado = "Trabalhadora com "+idade+ " anos e tempo de serviço "+tempoServico+ ". Resultado: apta para aposentadoria";
				} else {
					resultado = "Trabalhadora com "+idade+ " anos e tempo de serviço "+tempoServico+ ". Resultado: inapta para aposentadoria";
				}
			}
		}
		try {
